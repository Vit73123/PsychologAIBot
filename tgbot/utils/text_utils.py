...
def create_aboutme_string(user: UserDAO, user_upd: UserDAO, i18n: TranslatorRunner) -> str:
    ...
    gender_: str = user.gender if not user_upd.gender else user_upd.gender

    name = ' '.join([i18n.txt.name.before(), '<b>' + name_ + '</b>']) if name_ else ''
    age: str = ' '.join([i18n.txt.age.before(), '<b>' + str(age_) + '</b>', _after_years_string(years=age_, i18n=i18n)]) if age_ else ''
    gender: str = ' '.join([i18n.txt.gender.before(), '<b>' + gender_ + '</b>']) if gender_ else ''
