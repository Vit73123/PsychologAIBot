...
async def cmd_set_test(message: Message, dialog_manager: DialogManager, repo: Repo, state: FSMContext, **kwargs):
    log_dev.debug(' /user_reg: register user: state: %s, from_user: %s', await state.get_data(), message.from_user)
    ...
    if not data:
        ...
        log_dev.debug(' /user_reg: state: %s, user: %s', await state.get_data(), user)


@router.message(Command(commands='user_g'), IsAdmin())
async def cmd_get_test(message: Message, repo: Repo, state: FSMContext, **kwargs):
    log_dev.debug(' /user_g: get user: state: %s', await state.get_data())
...
async def cmd_get_by_bot_user_id_test(message: Message, repo: Repo, **kwargs):
    ...
    log_dev.debug(' /user_gg: user: %s', user)
...
async def cmd_get_with_all_statuses_test(message: Message, state: FSMContext, repo: Repo, **kwargs):
    log_dev.debug(' /user_ga: get with all statuses by user id=1: from_user: %s', message.from_user)
...
@router.message(Command(commands='user_ag'), IsAdmin())
async def cmd_add_test(message: Message, repo: Repo, **kwargs):
    log_dev.debug(" /user_g: add user gender user_id=9999999999 gender=%s", Gender.m)

    user = User()
    user.user_id = 1111111112
    user.username = 'New user gender name'
...
async def cmd_update_test(message: Message, state: FSMContext, repo: Repo, **kwargs):
    log_dev.debug(' /user_u: update user id=1, name=Vladimir')
    ...
    log_dev.debug(' /user_u: user: %s', await repo.user.get(user.id))
...
async def cmd_delete_test(message: Message, repo: Repo, state: FSMContext, **kwargs):
    log_dev.debug(' /user_d: delete user id=1: from_user: %s', message.from_user)
...
async def cmd_get_with_last_status_test(message: Message, state: FSMContext, repo: Repo, **kwargs):
    log_dev.debug(' /user_gl: get with last status by user id=1: from_user: %s', message.from_user)
    ...
    # log_dev.debug(' /user_ga: user: %s', user)
    pprint(user)
    # log_dev.debug(' /user_ga: status: %s', user.status)
